...
\end{itemize}
with plans to support additional profilers, including \pkg{TAU}~\citep{TAU}.
...
\code{pbdPROF/src/fpmpi} of the \pkg{pbdPROF} source. Although we bundle \pkg{pbdPROF} with \pkg{fpmpi}, it is not the best MPI profiler (though it may be sufficient for your needs).  The results from other libraries, such as \pkg{mpiP}, are much more thorough and may lead to much deeper insights.  However, \pkg{fpmpi} is the easiest to install. 
